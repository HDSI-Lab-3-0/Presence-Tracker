  ...
  handler: httpAction(async (ctx, request) => {
    try {
      ...
      return jsonResponse(200, {
        ...
        bluetoothStatus: result?.bluetoothStatus,
        attendanceDriver: result?.attendanceDriver ?? "bluetooth",
  ...
  handler: httpAction(async (ctx, request) => {
    try {
      ...
      return jsonResponse(200, {
        ...
        attendanceStatus: result?.attendanceStatus || "absent",
        attendanceChangedAt: result?.attendanceChangedAt ?? null,
        attendanceOrigin: result?.attendanceOrigin || "bluetooth",
        attendanceVerificationStatus: result?.attendanceVerificationStatus || "unverified",
        attendanceVerifiedBy: result?.attendanceVerifiedBy || "none",
        latestAppIntent: result?.latestAppIntent ?? null,
        latestAppIntentAt: result?.latestAppIntentAt ?? null,
        pendingVerificationAction: result?.pendingVerificationAction ?? null,
        pendingVerificationExpiresAt: result?.pendingVerificationExpiresAt ?? null,
        bluetoothStatus: result?.bluetoothStatus || "absent",
        lastBluetoothPresentAt: result?.lastBluetoothPresentAt ?? null,
        lastBluetoothAbsentAt: result?.lastBluetoothAbsentAt ?? null,
        keyVersion: result?.keyVersion,
        attendanceDriver: result?.attendanceDriver ?? "bluetooth",
